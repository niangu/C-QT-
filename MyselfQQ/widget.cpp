...
void Widget::processPendingDatagrams()
{
    while(udpSocket->hasPendingDatagrams())
    {
        ...
        case Msg:
            ...
            if(ui->checkBox_RSA->isChecked())
            {
                ui->msgBrowser->append(rsa->word);

            }
            if(ui->checkBox_MD5->isChecked())
            {
                ui->msgBrowser->append(MD5->MyMD5);
                qDebug()<<MD5->MyMD5;
                qDebug()<<"选中MD5";
            }
            break;
...
QString Widget::getIP()//获取用户登录时的IP地址
{
...
void Widget::on_pushButton_MD5_clicked()
{
    MD5=new MD5_check;
    MD5->show();
}

void Widget::on_pushButton_rsa_clicked()
{
    rsa=new QtRSA;
    rsa->show();
}
...
void Widget::on_pushButton_sendRSA_clicked()
{
    userRSA();
    qDebug()<<rsa->word;
    sndMsg(usrRSA);
